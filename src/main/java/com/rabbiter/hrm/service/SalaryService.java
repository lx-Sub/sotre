...
import com.rabbiter.hrm.mapper.AttendanceMapper;
import com.rabbiter.hrm.mapper.SalaryMapper;
    ...
    private void setSalaryInfo(String month, List<StaffSalaryVO> list) {
        for (StaffSalaryVO staffSalaryVO : list) {
            ...
            if (one != null) {
                ...
                staffSalaryVO.setSubsidy(one.getSubsidy());
                staffSalaryVO.setBonus(one.getBonus());
                staffSalaryVO.setRemark(one.getRemark());
                staffSalaryVO.setTotalSalary(one.getBaseSalary()
                        .add(one.getBonus())
                        .add(one.getSubsidy())
                        .subtract(lateDeduct)
